...
const packagePath = path.join(repoRoot, 'packages/jobs/package.json');
const packageJson = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
const packageName = packageJson.name;
const version = process.argv[2] ?? packageJson.version;
const tempRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), 'nest-native-jobs-published-release-'),
);
...
function writeConsumerPackage(consumerRoot) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: 'nest-native-jobs-published-consumer',
        private: true,
        type: 'commonjs',
        dependencies,
      },
...
// math for schedules — see the guidelines' constitution amendment; consumers only pull
// the peers they actually use).
assert.deepEqual(
  Object.keys(packageJson.dependencies ?? {}),
  ['croner'],
  'The published package must declare exactly one runtime dependency: croner.',
);
...
function writeSampleWorkspacePackage(sampleWorkspace) {
  fs.writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: 'nest-native-jobs-published-samples',
        private: true,
        workspaces: ['sample/*'],
        scripts: {
          showcase:
            'node scripts/run-optional-workspace.cjs nest-native-jobs-showcase test',

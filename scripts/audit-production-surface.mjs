...
const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-jobs-audit-cache-'));
const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-jobs-audit-consumer-'));
...
try {
  ...
  npm(['run', 'build', '--workspace', '@nest-native/jobs'], { cwd: repoRoot, stdio: 'inherit' });

  const packOutput = npm(
    ['pack', '--json', '--workspace', '@nest-native/jobs', '--pack-destination', consumerDir],
    { cwd: repoRoot },
  );
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: 'nest-native-jobs-audit-consumer', version: '0.0.0', private: true }, null, 2)}\n`,
  );
